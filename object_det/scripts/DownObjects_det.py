...
class Depth_Estimate:
    def __init__(self,img2_topic):
        self.yolo_detector = Yolo_Detect(Model_path)
        self.img_bridge = CvBridge()
        #/home/nvidia/catkin_ws/src/Challege_ROS/object_det/scripts/test_node.py
        ...
        self.ret_pub = rospy.Publisher("/objects2",Objects,queue_size=10)
        # self.ret_pub = rospy.Publisher("/obj2",Obj,queue_size=10)
    ...
    def run(self):
        ...
        if len(det) != 0:
            ...
            # obj.class_id = det[frame_index,5]
            # self.ret_pub.publish(obj)
...
if __name__ == "__main__":
    rospy.init_node('land_det_node')
    rgb_topic = "/rflysim/sensor2/img_rgb"
    img_proc = Depth_Estimate(img2_topic = rgb_topic)
    while True:
        img_proc.run()
